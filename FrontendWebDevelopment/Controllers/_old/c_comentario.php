...
if (isset($_POST['AddComentario'])) {
    ...
    if (cadastrarComentario($conn, $dados)) {
        header("location:" . SITE_URL . "/Views/trade_posts/trade_post_view.php?produto=" . $_POST['cod_produto']);
    } else {
        /** max 250 char */
...
if (isset($_GET['cod_comentario'])) {
   $codComentario = $_GET['cod_comentario'];
    if (deletarComentario($conn, $codComentario)) {
        header("location:" . SITE_URL . "/Views/trade_posts/trade_post_view.php?produto=" . $_GET['cod_produto']);
    } else {
